...
** Last update Sat Dec 15 12:18:08 2012 ivan ignatiev
*/
...
static int	cw_ldi_args(t_program *prog, op_t *instr, t_prog_args *args)
{
  t_long_type	addr;
  ...
  if (cw_get_args(prog, instr, args))
	...
	args[1].wval = prog->reg[(args[1].value - 1)];
      addr = cw_m(prog->previos_pc + ((prog->previos_pc
				       + (args[0].wval % IDX_MOD))
				      + args[1].wval) % IDX_MOD);
      cw_frommemcpy(&args[2].wval, REG_SIZE,
		    sizeof(args[2].wval), addr);
      my_conv_to_platform(&args[2].wval, sizeof(args[2].wval));
      args[2].change = 1;
...
{
  if (prog->instr.wait)
    return (cw_ldi_args(prog, instr, args));
